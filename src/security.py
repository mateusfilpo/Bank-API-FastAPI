...
async def decode_jwt(token: str) -> JWTToken | None:
    try:
        ...
        # print(f"Sub Field Type: {type(decoded_token.get('sub'))}")

        # Garantir que o sub seja tratado como string
        if "sub" in decoded_token and not isinstance(decoded_token["sub"], str):
            decoded_token["sub"] = str(decoded_token["sub"])

        # Criar o modelo AccessToken a partir do decoded_token
        access_token = AccessToken(
        ...
        # Agora podemos retornar o JWTToken, incluindo o access_token
        return (
